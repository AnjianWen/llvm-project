...
#include "MCTargetDesc/RISCWInstPrinter.h"
...
#define DEBUG_TYPE "riscw-asm-printer"

namespace llvm {
class RISCWAsmPrinter : public AsmPrinter {
public:
  explicit RISCWAsmPrinter(TargetMachine &TM,
...
#include "RISCWGenMCPseudoLowering.inc"
void RISCWAsmPrinter::EmitToStreamer(MCStreamer &S, const MCInst &Inst) {
  AsmPrinter::EmitToStreamer(*OutStreamer, Inst);
}

void RISCWAsmPrinter::emitInstruction(const MachineInstr *MI) {
...
void RISCWAsmPrinter::LowerInstruction(const MachineInstr *MI,
                                       MCInst &OutMI) const {
...
MCOperand RISCWAsmPrinter::LowerOperand(const MachineOperand& MO) const {
  switch (MO.getType()) {
...
MCOperand RISCWAsmPrinter::LowerSymbolOperand(const MachineOperand &MO,
                                              MCSymbol *Sym) const {
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeRISCWAsmPrinter() {
  RegisterAsmPrinter<RISCWAsmPrinter> X(getTheRISCWTarget());
}
